...
# ============================================================
# QuickTUI Installer
# Usage: curl -fsSL https://quicktui.ai/q-legacy.sh | sh
# ============================================================
...
QUICKTUI_MANIFEST_URL_ENV_SET="${QUICKTUI_MANIFEST_URL+x}"
QUICKTUI_MANIFEST_URL_ENV_VALUE="${QUICKTUI_MANIFEST_URL:-}"
QUICKTUI_RELEASES="https://github.com/${QUICKTUI_REPO}/releases/latest/download"
# Frozen copy of the built-in GitHub stable default. Used to gate manifest
# resolution: the CN build (generate-q-sh-cn.sh) rewrites the QUICKTUI_RELEASES
# line above to a controlled dl.quicktui.cn path but leaves this sentinel on
# github.com, so the CN script's QUICKTUI_RELEASES no longer equals the
# sentinel and stays on its static releases/latest path instead of resolving.
QUICKTUI_RELEASES_GITHUB_DEFAULT="https://github.com/${QUICKTUI_REPO}/releases/latest/download"
...
QUICKTUI_MANIFEST_URL="https://quicktui.ai/server-manifest.json"
...
[ -n "$QUICKTUI_MANIFEST_URL_ENV_SET" ] && QUICKTUI_MANIFEST_URL="$QUICKTUI_MANIFEST_URL_ENV_VALUE"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --uninstall)
            ...
            ;;
        -h|--help)
            printf 'Usage: q-legacy.sh [OPTIONS]\n\n'
            printf 'Options:\n'
            ...
            printf '  QUICKTUI_MANIFEST_URL         Server manifest used to resolve the stable release tag (default: %s)\n' "$QUICKTUI_MANIFEST_URL"
            ...
            printf '  curl -fsSL https://quicktui.ai/q-legacy.sh \\\n'
...
# Resolve the stable channel's release tag from the server manifest by
# default so q.sh does not depend on GitHub's repo-wide "latest release"
# pointer. That pointer is computed across every release in the repo, so a
# non-server release (installer, android) newer than the stable server
# release silently hijacks /releases/latest and 404s the server download.
# Skip resolution when the caller pinned an explicit releases base
# (QUICKTUI_RELEASES env, or the CN build whose QUICKTUI_RELEASES no longer
# matches the github sentinel and serves a controlled static path), an
# explicit --server-release tag, or --preview.
STABLE_VIA_MANIFEST=""
if [ -z "$QUICKTUI_RELEASES_ENV_SET" ] && [ -z "$PREVIEW_RELEASE" ] && [ -z "$OPT_SERVER_RELEASE" ] \
    && [ "$QUICKTUI_RELEASES" = "$QUICKTUI_RELEASES_GITHUB_DEFAULT" ]; then
    STABLE_VIA_MANIFEST="1"
fi
...
# Extract the stable channel's release tag from server-manifest.json using
# only awk (no jq dependency — q.sh must run on any POSIX host). The manifest
# is the JSON emitted by publish.sh, the same shape consumed by the Rust
# installer and server self-update. The whole file is read as one record (RS
# set to a byte that cannot appear in JSON text) so parsing works whether the
# manifest is pretty-printed or compact, and regardless of where line breaks
# fall. The match is anchored at the top-level "stable" key and takes the
# first "tag": "<value>" after it, so the "preview" block (which also carries
# a "tag", and may even appear before "stable") is never mistaken for stable.
extract_manifest_stable_tag() {
    _manifest_file="$1"
    awk '
        BEGIN { RS = "\1" }
        {
            s = $0; n = length(s)
            # Depth/string-aware scanner that extracts strictly .stable.tag:
            # the "stable" key is only honoured at object depth 1 (top level),
            # and "tag" only as a direct member of the stable object (the depth
            # at which the stable object body lives). This rejects an
            # incidental "stable" string in an array, a nested
            # meta.stable.tag, and a nested stable.history[].tag.
            depth = 0; in_str = 0; esc = 0; cur = ""
            pend = ""; pend_depth = 0; have_pend = 0
            want_stable = 0; in_stable = 0; stable_depth = 0
            cap_tag = 0; cap_now = 0
            i = 1
            while (i <= n) {
                c = substr(s, i, 1)
                if (in_str) {
                    if (esc) { cur = cur c; esc = 0; i++; continue }
                    if (c == "\\") { cur = cur c; esc = 1; i++; continue }
                    if (c == "\"") {
                        in_str = 0
                        if (cap_now == 1) { print cur; exit 0 }
                        pend = cur; pend_depth = depth; have_pend = 1
                        i++; continue
                    }
                    cur = cur c; i++; continue
                }
                if (cap_tag == 1) {
                    # Awaiting the tag value; only a string value is accepted.
                    if (c == " " || c == "\t" || c == "\n" || c == "\r") { i++; continue }
                    if (c == "\"") { in_str = 1; cur = ""; cap_now = 1; cap_tag = 0; i++; continue }
                    cap_tag = 0
                }
                if (want_stable == 1) {
                    # Only an object value enters the stable object. A scalar
                    # value (null / number / string / bool / array) means there
                    # is no stable object, so clear the pending flag before the
                    # next "{" (such as the preview block) is misread as stable.
                    if (c == " " || c == "\t" || c == "\n" || c == "\r") { i++; continue }
                    if (c != "{") { want_stable = 0 }
                }
                if (c == "\"") { in_str = 1; cur = ""; i++; continue }
                if (c == ":") {
                    if (have_pend) {
                        if (pend == "stable" && pend_depth == 1) { want_stable = 1 }
                        else if (pend == "tag" && in_stable && pend_depth == stable_depth) { cap_tag = 1 }
                        have_pend = 0
                    }
                    i++; continue
                }
                if (c == "{") {
                    depth++
                    if (want_stable == 1) { in_stable = 1; stable_depth = depth; want_stable = 0 }
                    have_pend = 0; i++; continue
                }
                if (c == "[") { depth++; have_pend = 0; i++; continue }
                if (c == "}") {
                    # Clamp at 0 so a stray closing brace in malformed input
                    # cannot drive depth negative and promote a nested
                    # stable.tag to the top level.
                    if (depth > 0) { depth-- }
                    if (in_stable && depth < stable_depth) { in_stable = 0 }
                    have_pend = 0; i++; continue
                }
                if (c == "]") { if (depth > 0) { depth-- } have_pend = 0; i++; continue }
                if (c == ",") { have_pend = 0; i++; continue }
                i++
            }
        }
    ' "$_manifest_file"
...
# Fetch the server manifest and repoint QUICKTUI_RELEASES at the stable
# release tag's per-tag download path. Uses the same download() and tag
# charset guard as the --server-release path. Requires DOWNLOAD_TMPDIR to
# already be set (download_binary creates it before calling this).
resolve_stable_release_base() {
    _manifest_path="${DOWNLOAD_TMPDIR}/server-manifest.json"
    download "$QUICKTUI_MANIFEST_URL" "$_manifest_path" "Resolving latest stable release..." || \
        die "Failed to fetch server manifest from ${QUICKTUI_MANIFEST_URL}."
    _stable_tag="$(extract_manifest_stable_tag "$_manifest_path")"
    [ -n "$_stable_tag" ] || \
        die "Server manifest at ${QUICKTUI_MANIFEST_URL} has no stable release tag."
    # Same charset guard as --server-release (kept broad so a future
    # server2-YYYYMMDD-NN stable tag still validates), plus an explicit
    # reject of "." / ".." path segments so a tampered manifest cannot
    # walk the download path up out of the releases tree.
    case "$_stable_tag" in
        *[!A-Za-z0-9._-]*)
            die "Server manifest stable tag '$_stable_tag' contains invalid characters."
            ;;
        .|..|*..*)
            die "Server manifest stable tag '$_stable_tag' is not a valid release tag."
            ;;
    esac
    QUICKTUI_RELEASES="${QUICKTUI_GITHUB_BASE}/releases/download/${_stable_tag}"
    info "Resolved stable release ${_stable_tag}"
}
    ...
    if [ -f "$QUICKTUI_CONFIG_FILE" ]; then
        while IFS='=' read -r _key _val; do
            case "$_key" in
                ...
                QUICKTUI_TMUX_SOCKET) EXISTING_TMUX_SOCKET="$_val" ;;
            esac
...
download_binary() {
    ...
    if [ -n "$PREVIEW_RELEASE" ]; then
        ...
        [ -n "$STABLE_VIA_MANIFEST" ] && resolve_stable_release_base
...
write_terminal_config() {
    ...
    if [ -n "$EXISTING_TMUX_SOCKET" ]; then
        printf 'QUICKTUI_TMUX_SOCKET=%s\n' "$EXISTING_TMUX_SOCKET" >> "$_CONFIG_TMP"
    else
        printf 'QUICKTUI_TMUX_SOCKET=/tmp/tmux-%s/default\n' "$(id -u)" >> "$_CONFIG_TMP"
    fi
...
configure_service() {
    ...
    _SVC_OUT="$(mktemp)"
    _svc_rc=0
    env -u QUICKTUI_TOKEN \
        -u QUICKTUI_ADDR \
        -u QUICKTUI_FRONTEND_DIR \
        -u QUICKTUI_TERM \
        -u QUICKTUI_LANG \
        -u QUICKTUI_TMUX_BIN \
        -u QUICKTUI_TMUX_SOCKET \
        -u QUICKTUI_SESSION_BACKEND \
        -u QUICKTUI_QSCN_BIN \
        -u QUICKTUI_QSCN_SOCKET \
        -u QUICKTUI_DEBUG \
        -u QUICKTUI_VERBOSE \
        -u QUICKTUI_CHAT_TRANSCRIPT_RETENTION_DAYS \
        "$INSTALL_PATH" --install-service \
        --config "$QUICKTUI_CONFIG_FILE" \
    ...
    awk '
        /Run .*--list-addr --qrcode.*display the connection QR code/ { next }
        { print }
    ' "$_SVC_OUT"
    rm -f "$_SVC_OUT"
    _SVC_OUT=""
...
print_manual_start_command() {
    ...
    _cfg="$(shell_quote "$QUICKTUI_CONFIG_FILE")"
    printf '  QUICKTUI_TOKEN=$(sed -n "s/^QUICKTUI_TOKEN=//p" %s | tail -n 1) ' "$_cfg"
    printf 'QUICKTUI_TMUX_SOCKET=$(sed -n "s/^QUICKTUI_TMUX_SOCKET=//p" %s | tail -n 1) ' "$_cfg"
...
print_native_qrcode() {
    ...
    if [ -c /dev/tty ] && ( : </dev/tty ) 2>/dev/null; then
        "$INSTALL_PATH" --config "$QUICKTUI_CONFIG_FILE" --list-addr --qrcode </dev/tty
    else
        "$INSTALL_PATH" --config "$QUICKTUI_CONFIG_FILE" --list-addr --qrcode
    fi
}
...
print_interactive_qrcode() {
    ...
    # can rerun on demand.
    printf '    Run %s%s --config %s --list-addr --qrcode%s to display the QR code.\n' \
        "$C_BOLD$C_GREEN" "$(shell_quote "$INSTALL_PATH")" "$(shell_quote "$QUICKTUI_CONFIG_FILE")" "$C_RESET"
}
...
print_success() {
    ...
    if [ "$SERVICE_STARTED" = "yes" ]; then
        ...
        if [ -z "$NON_INTERACTIVE" ]; then
            ...
            printf '    Run:\n'
            printf '      %s%s --config %s --list-addr --qrcode%s\n' \
                "$C_BOLD$C_GREEN" "$(shell_quote "$INSTALL_PATH")" "$(shell_quote "$QUICKTUI_CONFIG_FILE")" "$C_RESET"
